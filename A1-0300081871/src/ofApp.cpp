...
void  ofApp::processAxiom()
{
 ...
 int where = -1; //store location in the "parallel" vectors rule and rule

 for(auto ch : axiom) //for each character in axiom
  {
     where = linearSearch(rules,ch);//look for ch in Rule vector
     
     if (where == -1)
     {
       str += ch; //ch was not found so its value is not changed in new string    
     }
    else // ch was found in Rule vector
     {
      ...
      for(auto i = 0u; i < R; i++)
         {
           ...
           RUOB.SettoString(str);
           RUOB.setfromChar(ch);
rules.push_back(RUOB);
         }
